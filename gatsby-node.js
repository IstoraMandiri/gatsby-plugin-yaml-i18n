...
exports.onCreateNode = async ({
  ...
  const keystore = node.relativePath.startsWith('_i18n/')
...
exports.createPages = async ({ graphql, actions: { createPage } }, passedConfig) => {
  ...
  if (pages.length === 0) {
    throw Error(
      `Could not find any content! Did you specify 'gatsby-source-filesystem' with name: '${CONTENT_KEY}'?`
    )
  }
  if (_templates.length === 0) {
    throw Error(
      `Could not find any templates! Did you specify 'gatsby-source-filesystem' with name: '${TEMPLATES_KEY}'?`
    )
  }
  // transform into key / vals
  const templates = _templates.reduce(
    (o, { relativePath: r, absolutePath: a }) => ({ ...o, [r]: a }),
    {}
  )
  // get translations
  ...
  pages.forEach(({ relativePath }) => {
    ...
    if (!component) {
      throw Error(
        `Could not find template for ${relativePath}. Did you create a ${DEFAULT_TEMPLATE} file?`
      )
    }
    locales.forEach((locale) => {
      ...
      const globals = isDefaultLocale
        ? defaultGlobals
        : utils.merge(defaultGlobals, utils.getGlobals(translations.global, relativePath, locale))
      const _locals = isDefaultLocale
        ? defaultLocals
        : utils.merge(defaultLocals, (translations.local[relativePath] || {})[locale])
      const { mdxId, ...locals } = _locals
      // skip markdown page creation if it wasn't translated, unless overridden
      if (
        utils.skipGeneration({ locals: _locals, isDefaultLocale, relativePath, generateMissing })
      ) {
        return
      }
      const pageData = {
